...
function SearchReportFields({ tenantId, t, control, handleSubmit, reset, handleClick }) {
  // ---- this has to change according to the module --->
  const { data: applicationTypes, isLoading: applicationTypesLoading } = Digit.Hooks.tl.useMDMS.applicationTypes(tenantId);

  // ----- for testing data on console ----->
  ...
  return (
    <>
      {/* {applicationTypesLoading ? (
        ...
        <SearchField>
          <label>{t("PET_LOCALIZATION_PET_TYPE")}</label>
          <Controller
            control={control}
...
export default SearchReportFields;

// import React, { Fragment } from "react";
// import { Controller, useWatch } from "react-hook-form";
// import { TextInput, SubmitBar, DatePicker, SearchField, Dropdown, Loader } from "@egovernments/digit-ui-react-components";

// const SearchReportFields = ({ register, control, reset, tenantId, t, handleSubmit, previousPage }) => {
//   const { data: applicationTypes, isLoading: applicationTypesLoading } = Digit.Hooks.tl.useMDMS.applicationTypes(tenantId);
...
//       )}
//       <SearchField>
//         <label>{t("PET_APPLICATION_FROM_DATE")}</label>
//         <Controller render={(props) => <DatePicker date={props.value} onChange={props.onChange} />} name="fromDate" control={control} />

//       </SearchField>
//       <SearchField>
//         <label>{t("PET_APPLICATION_TO_DATE")}</label>
//         <Controller render={(props) => <DatePicker date={props.value} onChange={props.onChange} />} name="toDate" control={control} />
